...
public abstract class SecurityManager implements IAuthorization,ISecurityManager {
	...
	private void setupValidation(RSAKey jwk) throws JOSEException {		
		...
		Logging.debug("Public key to validate JWT");
		Logging.debug(jwkPublicKey.toString());
	...
	public void securityCheck(String identity) throws SEPASecurityException {
		...
		if (response.getClass().equals(RegistrationResponse.class)) {
			...
			Logging.debug("Authorization Basic " + encodedCredentials);
			response = getToken(encodedCredentials);

			if (response.getClass().equals(JWTResponse.class)) {
				Logging.debug("Access token: " + ((JWTResponse) response).getAccessToken());
				...
				if (authRet.isAuthorized()) {
					...
					Logging.error(authRet.getError());
					Logging.info("*** FAILED ***");
				}
			} else {
	...
	public synchronized Response register(String uid) {
		Logging.info("REGISTER: " + uid);

		// Check if entity is authorized to request credentials
		try {
			if (!isAuthorized(uid)) {
				Logging.warn("Not authorized identity " + uid);
	...
	public synchronized Response getToken(String encodedCredentials) {
		...
		if (marker == -1) {
			Logging.error("Wrong Basic authorization");
...
//		String secret = decodedCredentials.split(":")[1];
		...
		Logging.debug("Credentials: " + id + " " + secret);

		// Verify credentials
		try {
			if (!containsCredentials(id)) {
				Logging.error("Client id: " + id + " is not registered");
		...
		try {
			if (!checkCredentials(id, secret)) {
				Logging.error("Wrong secret: " + secret + " for client id: " + id);
		...
		try {
			if (containsJwt(id)) {
				...
				Logging.debug("ID: " + id + " ==> Token will expire in: " + delta + " ms");
				if (delta > 0) {
					Logging.warn("Token is NOT EXPIRED. Return the current token.");
				...
				Logging.debug("Token is EXPIRED. Release a fresh token.");
			}
		} catch (SEPASecurityException e2) {
...
//		claimsSetBuilder.audience(audience);
		...
		Logging.debug("Released token: " + jwt);
	...
	public synchronized ClientAuthorization validateToken(String accessToken) {
		...
		try {
			if (!signedJWT.verify(verifier)) {
				Logging.error("Signed JWT not verified");
		...
		if (expiring.getTime() - nowUnixSeconds < 0) {
			Logging.warn("Token is expired: " + sdf.format(claimsSet.getExpirationTime()) + " < "
		...
		if (notBefore != null && nowUnixSeconds < notBefore.getTime()) {
			Logging.warn("Token can not be used before: " + claimsSet.getNotBeforeTime());
		...
		Logging.debug("Get credentials for uid: "+id);
		
		Credentials cred = null;
		try {
			cred = getEndpointCredentials(id);
			Logging.trace(cred.toString());
		} catch (SEPASecurityException e) {
			Logging.error("Failed to retrieve credentials ("+id+")");
